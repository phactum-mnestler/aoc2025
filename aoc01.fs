...
module AOC01 =
    ...
    let parse (expr: string) = 
        ...
        then int(expr.Substring(1)) 
        else -int(expr.Substring(1))
    ...
    let points (before, after) =
        if after > before 
        then int(floor (float(after) / 100.0) - floor (float(before) / 100.0))
        else if before > after 
        then int(floor ((float(before) - 1.0) / 100.0) - floor ((float(after) - 1.0) / 100.0))
        else 0
